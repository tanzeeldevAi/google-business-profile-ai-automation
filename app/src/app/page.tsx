...
import { Button, Card, Dial, Empty, Pill, Spark } from "@/components/ui";
import { ago, api, Audit, SEVERITY_STYLE } from "@/lib/api";
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <div className="lg:col-span-2 space-y-5">
          ...
          {audit ? (
            <div className="flex items-center gap-5 flex-wrap">
              ...
              <div>
                ...
                <div className="text-xs text-ink-3 mt-1">Checked {ago(audit.when)}</div>
              </div>
        ...
        <Card title={`What to fix (${failures.length})`}
              right={<Link href="/audit" className="text-xs text-accent">See all →</Link>}>
          {failures.length === 0 ? (
            <Empty>{audit ? "Nothing failed on this profile." : "Run an audit first."}</Empty>
          ) : (
            <div className="space-y-2">
              {failures.slice(0, 6).map((f) => (
                <div key={f.rule_id} className="flex gap-3 items-start p-3 rounded-lg bg-panel-2">
                  <span className={`text-[10.5px] uppercase tracking-wider px-2 py-0.5 rounded border shrink-0 ${SEVERITY_STYLE[f.severity]}`}>
                    {f.severity}
                  </span>
                  ...
                  {f.fixable && <span className="ml-auto shrink-0"><Pill tone="good">auto</Pill></span>}
                </div>
              ))}
            </div>
          )}
      ...
      <div className="space-y-5">
        <Card title="Autopilot">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button kind="write" disabled={running}
        ...
        {alerts.length > 0 && (
          <Card title={`Alerts (${alerts.length})`}
                right={<button className="text-xs text-accent"
                               onClick={() => api.ackAlerts(active.location).then(() => setAlerts([]))}>
                        Mark seen</button>}>
            <div className="space-y-2">
              {alerts.map((a, i) => (
                <div key={i} className="text-sm p-2.5 rounded-lg bg-bad/10 border border-bad/25 text-bad">
                  {a.message}
                </div>
              ))}
            </div>
              ...
              {reports.slice(0, 5).map((r) => (
                <li key={r.name}>
                  <a href={api.reportUrl(r.name)} target="_blank" rel="noreferrer"
                     className="text-accent hover:underline break-all">{r.name}</a>
                  <span className="text-ink-3 text-xs"> · {ago(r.when)}</span>
                </li>
              ))}
            </ul>
          )}
...
function Row({ ok, label, hint }: { ok?: boolean; label: string; hint?: string }) {
  return (
    <li className="flex items-center gap-2">
      ...
      {!ok && hint && <span className="text-xs text-ink-3 ml-auto">{hint}</span>}
    </li>
  );
}
